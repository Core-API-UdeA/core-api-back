module.exports = {
  ...
  description:
    "Crea una preferencia de pago en Mercado Pago y retorna el link de checkout",
  ...
  fn: async function ({ transaction, plan, api }) {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "No se pueden usar URLs localhost en modo producción. Usa ngrok o un dominio público."
        );
      }
      ...
      const items = [
        {
          ...
          description: (
            plan.description ||
            `Suscripción al plan ${plan.name} de ${api.title}`
          ).substring(0, 256),
      ...
      const backUrls = {
        success: `${sails.config.frontendUrl}/payment/success?transaction_id=${transaction.id}`,
        failure: `${sails.config.frontendUrl}/payment/cancel?transaction_id=${transaction.id}`,
        pending: `${sails.config.frontendUrl}/payment/pending?transaction_id=${transaction.id}`,
      };
      ...
      const preferenceData = {
        ...
        payer: {
          email: isSandbox
            ? "test_user_46924593@testuser.com"
        ...
        notification_url: `${sails.config.baseUrl}${
          sails.config.prefix || ""
        }/webhooks/mercadopago`.trim(),
      ...
      await ApiTransaction.updateOne({ id: transaction.id }).set({
