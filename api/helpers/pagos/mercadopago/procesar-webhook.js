module.exports = {
  ...
  fn: async function ({ type, data }) {
    ...
    try {
      ...
      const isSandbox =
        sails.config.mercadoPagoAccessToken?.includes("TEST-") ||
        sails.config.environment === "development";

      switch (type) {
        case "payment":
          await module.exports._private.procesarPago(
            client,
            data.id,
            isSandbox,
          );
        ...
        case "merchant_order":
          await module.exports._private.procesarOrden(
            client,
            data.id,
            isSandbox,
          );
      ...
      if (error.status === 404 || error.error === "not_found") {
        sails.log.warn(
          "Recurso no encontrado en MP (puede ser timing o sandbox), se ignorará",
        );
  ...
  try {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        sails.log.verbose(
          `Pago ${paymentId} encontrado en intento ${attempts + 1}`,
        );
        ...
        if (error.status === 404 || error.error === "not_found") {
          if (attempts < maxAttempts) {
            sails.log.verbose(
              `Pago ${paymentId} no encontrado, reintento ${attempts}/${maxAttempts} en ${retryDelay}ms...`,
            );
            ...
            if (isSandbox) {
              sails.log.warn(
                `Modo sandbox: Intentando procesar sin consultar a MP...`,
              );
              return await module.exports._private.procesarPagoSandboxFallback(paymentId);
            } else {
              sails.log.warn(
                `Pago ${paymentId} no encontrado después de ${maxAttempts} intentos.`,
              );
              throw error;
            }
          }
    ...
    await module.exports._private.procesarPagoConInfo(paymentInfo);
  } catch (error) {
  ...
  try {
    ...
    await Api.getDatastore().transaction(async (db) => {
      ...
      if (transaccionesPendientes.length === 0) {
        sails.log.warn(
          "No hay transacciones pendientes recientes para procesar",
        );
        return;
      }
      ...
      sails.log.verbose(
        `Procesando transacción ${transaction.id} en modo sandbox`,
      );
      ...
      sails.log.verbose(
        `Transacción actualizada (sandbox): ${transaction.id} -> completed`,
      );

      // Crear suscripción si no existe
      if (!transaction.subscription_id) {
        await module.exports._private.crearSuscripcionDesdePago(transaction, db);
      }
    });
  } catch (error) {
...
async function procesarPagoConInfo(paymentInfo) {
  ...
  await Api.getDatastore().transaction(async (db) => {
    ...
    if (
      ...
      sails.log.verbose(
        `Transacción ${transactionId} ya fue procesada, ignorando webhook duplicado`,
      );
      return;
    }
    ...
    await ApiTransaction.updateOne({ id: transactionId })
      .set({
        ...
        payment_metadata: {
          ...
          webhook_processed_at: new Date(),
          webhook_attempts:
            (transaction.payment_metadata?.webhook_attempts || 0) + 1,
        },
...
async function procesarOrden(client, orderId, isSandbox = false) {
  ...
  try {
    ...
    if (error.status === 404 || error.error === "not_found") {
      sails.log.warn(
        `Orden ${orderId} no encontrada en MP (puede ser timing issue o sandbox)`,
      );
      return;
    }
...
async function crearSuscripcionDesdePago(transaction, db) {
  ...
  try {
    ...
    switch (plan.billing_cycle) {
      ...
      default:
        sails.log.warn(
          `Ciclo de facturación no reconocido: ${plan.billing_cycle}`,
        );
        endDate = new Date(startDate);
        endDate.setMonth(endDate.getMonth() + 1);
        nextBillingDate = new Date(endDate);
    ...
    if (existingSubscription) {
      sails.log.verbose(
        `Ya existe una suscripción activa, vinculando con la transacción`,
      );
    ...
    const subscription = await ApiSubscription.create({
      ...
      auto_renew:
        ...
        plan.billing_cycle !== "pay_per_use",
      api_key: apiKey,
...
module.exports._private = {
  procesarPago,
  procesarPagoSandboxFallback,
  procesarPagoConInfo,
  procesarOrden,
  crearSuscripcionDesdePago,
};
